...
export default function MetaDetalhes() {
  ...
  // Alternar concluído de uma micro-meta
  async function toggleConcluido(micrometa: MicroMeta) {
    if (!session?.user) return;

    try {
      setUpdatingId(micrometa.id);

      const { error } = await supabase
        .from("micro_metas")
        .update({ concluido: !micrometa.concluido })
        .eq("id", micrometa.id)
        .eq("meta_id", metaId);
      ...
      if (!error) {
        setMicroMetas((prev) =>
          prev.map((m) =>
            m.id === micrometa.id
              ? { ...m, concluido: !micrometa.concluido }
              : m
          )
        );
      } else {
        ...
        <div className="flex flex-col gap-4">
          ...
          >
            ...
            <div className="flex flex-wrap gap-2 text-xs">
              ...
              {meta.inicio && (
                <span className="px-3 py-1 rounded-full bg-gray-700/40 text-gray-200 border border-gray-700 text-[11px]">
                  Início:{" "}
                  {new Date(meta.inicio).toLocaleDateString("pt-BR", {
                    day: "2-digit",
                    month: "2-digit",
                    year: "2-digit",
                  })}
              ...
              {meta.fim && (
                <span className="px-3 py-1 rounded-full bg-gray-700/40 text-gray-200 border border-gray-700 text-[11px]">
                  Fim previsto:{" "}
                  {new Date(meta.fim).toLocaleDateString("pt-BR", {
                    day: "2-digit",
                    month: "2-digit",
                    year: "2-digit",
                  })}
        ...
        <section className="space-y-3">
          ...
          <div className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-gray-50/70 dark:bg-gray-900/70 p-4 space-y-3">
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400">
              Cada micro-meta marcada como concluída aproxima você da meta
              principal. Mantenha a consistência ✨
            </p>
          </div>
        </section>
        ...
        <section className="space-y-4">
          ...
          {microMetas.length === 0 && (
            <p className="text-gray-500 text-sm">
              Nenhuma micro-meta criada ainda. Comece criando pequenas ações
              semanais ligadas a esta meta.
            </p>
          )}
                    ...
                    <div className="space-y-1">
                      ...
                      <p
                        className={`text-sm font-semibold ${
                          concluida ? "line-through text-emerald-100" : ""
                        }`}
                      >
                        {m.titulo}
                      </p>
                    ...
                    <div className="flex-shrink-0">
                      ...
                      >
                        {updatingId === m.id ? (
                          ...
                          <CheckCircle2
                            className={`w-4 h-4 ${
                              concluida ? "fill-white" : "fill-transparent"
                            }`}
                          />
                        )}
                      </Button>
