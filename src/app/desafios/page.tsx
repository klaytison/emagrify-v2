...
export default function DesafiosSemanaisPage() {
  ...
  async function concluirMissao(missaoId: string) {
    if (!session?.user || !desafio) return;

    try {
      setMarcando(missaoId);
      ...
      if (!res.ok || data?.error) {
        alert(data?.error || "Não foi possível concluir a missão.");
        return;
      }

      // Atualiza o estado localmente (sem precisar dar reload)
      setDesafio((prev) => {
        if (!prev) return prev;

        const xpMissao =
          prev.missoes.find((m) => m.id === missaoId)?.recompensaXp ?? 0;

        const novoXp =
          typeof data?.xp_atual === "number"
            ? data.xp_atual
            : prev.xpAtual + xpMissao;

        return {
          ...prev,
          xpAtual: novoXp,
          missoes: prev.missoes.map((m) =>
            m.id === missaoId ? { ...m, concluida: true } : m
          ),
        };
      });
    } catch (err) {
      console.error("Erro ao concluir missão:", err);
      alert("Erro inesperado ao concluir missão.");
    } finally {
      setMarcando(null);
    }
  }

  // ================== CARREGAMENTO DO DESAFIO ==================
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        // 1) Verificar se já existe desafio criado (último por ano/semana)
        ...
        let desafioID = desafioAtual?.id as string | undefined;

        // 2) Se não existir desafio, chamar função de IA para gerar um
        if (!desafioAtual) {
          const hoje = new Date();
          const ano = hoje.getFullYear();

          // Número simples da semana (não precisa ser perfeito)
          ...
          desafioID = novoDesafio.id as string;
        }
        ...
        // 3) Carregar informações do desafio (tabela DESAFIOS)
        ...
        // 5) Status geral do usuário nesse desafio (xp, nível etc.)
        ...
        // 6) Status individual de cada missão (tabela desafios_missoes_status)
        const { data: missoesStatus, error: erroStatus } = await supabase
        ...
        if (erroStatus) {
          console.error("Erro carregando desafios_missoes_status:", erroStatus);
        }

        const statusMap = new Map<string, boolean>();
        (missoesStatus || []).forEach((s: any) => {
          if (s.concluida) statusMap.set(s.missao_id, true);
        });

        // 7) Montar objeto no formato esperado pelo front
        const missoesConvertidas: Missao[] = (missoes as any[]).map((m) => {
          const concluidaIndividual = statusMap.get(m.id) ?? false;

          const concluida =
            concluidaIndividual ||
            completouTudo ||
            (completouTreinosObrigatorios && m.obrigatoria);
                    ...
                    >
                      ...
                      <div className="flex flex-col items-end gap-2">
                        ...
                        <Button
                          ...
                          disabled={m.concluida || marcando === m.id}
                          className="bg-emerald-600 hover:bg-emerald-700 text-xs disabled:opacity-60"
                        >
                          {m.concluida
                            ? "Já concluída"
                            : marcando === m.id
                    ...
                    >
                      ...
                      <div className="flex flex-col items-end gap-2">
                        ...
                        <Button
                          ...
                          onClick={() => concluirMissao(m.id)}
                          disabled={m.concluida || marcando === m.id}
                          className="bg-violet-600 hover:bg-violet-700 text-xs disabled:opacity-60"
                        >
                          {m.concluida
                            ? "Já concluída"
                            : marcando === m.id
